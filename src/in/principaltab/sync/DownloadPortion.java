...
import in.schoolcom.sqlite.Portion;
...
public class DownloadPortion implements SyncUrl{
	...
	static private int porSuccess = 0;
	static private List<Portion> aList;
	static private int reqCall=1,reqAgain=1;
	static private JSONObject jsonReceived;
	
	public DownloadPortion(Context con_text){
		context = con_text;
	...
	static class CalledDownloadPortion extends AsyncTask<String, String, String>{
		FirstTimeSyncParser jsonParser = new FirstTimeSyncParser();
		
		protected void onPreExecute(){
			super.onPreExecute();
			SyncTime.updateSyncProgress(context);
			pDialog.setMessage("Downloading 4 of 19 ...");
		...
		protected String doInBackground(String... arg0) {
			...
			aList = new ArrayList<Portion>();
			List<NameValuePair> param = new ArrayList<NameValuePair>();
			param.add(new BasicNameValuePair("tab_id", deviceId));
			param.add(new BasicNameValuePair("table_name", "portion"));
			...
			if(jsonReceived!=null){
				JSONArray sync = null;
				try{
					porSuccess = jsonReceived.getInt(TAG_SUCCESS);
					if(porSuccess == 1){
						...
						for(int i=0; i<sync.length(); i++){
							...
							p.setClassId(j.getInt("ClassId"));
							p.setNewSubjectId(j.getInt("NewSubjectId"));
							p.setPortion(j.getString("Portion"));
							p.setPortionId(j.getInt("PortionId"));
							p.setSchoolId(j.getInt("SchoolId"));
							p.setSubjectId(j.getInt("SubjectId"));
							aList.add(p);					
						}
						sqlHandler.initPortion(aList);
					}
		...
		protected void onPostExecute(String s){
			...
			if(porSuccess == 0){
				...
				SyncTime.updateSyncProgress(context);
				if(reqAgain!=0){
					new CalledDownloadPortion().execute();
				}else{
					new DownloadSchool(context).DownSchool();
				}
	...
	public void DownPortion(){
		sqlHandler.deleteTable("portion");
		new CalledDownloadPortion().execute();
	}
