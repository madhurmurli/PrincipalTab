...
import in.schoolcom.sqlite.Subjects;
...
public class DownloadSubjects implements SyncUrl{
	...
	static private int subSuccess = 0;
	static private List<Subjects> sList;
	static private int reqCall=1,reqAgain=1;
	static private JSONObject jsonReceived;
	
	public DownloadSubjects(Context con_text){
		context = con_text;
	...
	static class CalledDownloadSubjects extends AsyncTask<String, String, String>{
		FirstTimeSyncParser jsonParser = new FirstTimeSyncParser();
		
		protected void onPreExecute(){
			super.onPreExecute();
			SyncTime.updateSyncProgress(context);
			pDialog.setMessage("Downloading 9 of 19 ...");
		...
		protected String doInBackground(String... arg0) {
			...
			sList = new ArrayList<Subjects>();
			List<NameValuePair> param = new ArrayList<NameValuePair>();
			param.add(new BasicNameValuePair("tab_id", deviceId));
			param.add(new BasicNameValuePair("table_name", "subjects"));
			...
			if(jsonReceived!=null){
				JSONArray sync = null;
				try{
					subSuccess = jsonReceived.getInt(TAG_SUCCESS);
					if(subSuccess == 1){
						...
						for(int i=0; i<sync.length(); i++){
							...
							s.setSubjectName(j.getString("SubjectName"));							
							sList.add(s);		
						}
						sqlHandler.insertSubjects(sList);
					}
		...
		protected void onPostExecute(String s){
			...
			if(subSuccess == 0){
				...
				SyncTime.updateSyncProgress(context);
				if(reqAgain!=0){
					new CalledDownloadSubjects().execute();
				}else{
					new DownloadSubjectTeachers(context).DownSubjectTeachers();
				}
	...
	public void DownSubjects(){
		sqlHandler.deleteTable("subjects");
		new CalledDownloadSubjects().execute();
	}
