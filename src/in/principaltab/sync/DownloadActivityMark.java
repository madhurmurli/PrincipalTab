...
import in.schoolcom.sqlite.ActivityMark;
...
public class DownloadActivityMark implements SyncUrl{
	...
	static private int actMarkSuccess = 0;
	static private List<ActivityMark> amList;
	static private int reqCall = 1,reqAgain = 0;
	static private JSONObject jsonReceived;
	
	public DownloadActivityMark(Context con_text){
		context = con_text;
	...
	private static class CalledDownloadActivityMark extends AsyncTask<String, String, String>{
		FirstTimeSyncParser jsonParser = new FirstTimeSyncParser();
		
		protected void onPreExecute(){
			super.onPreExecute();
			SyncTime.updateSyncProgress(context);
			pDialog.setMessage("Downloading 14 of 19 ...");
		...
		protected String doInBackground(String... arg0) {
			...
			amList = new ArrayList<ActivityMark>();
			...
			if(jsonReceived!=null){
				...
				try{
					...
					if(actMarkSuccess == 1){
						...
						sqlHandler.initActivityMark(amList);
					}
				}
		...
		protected void onPostExecute(String s){
			...
			if(actMarkSuccess == 0){
				...
				SyncTime.updateSyncProgress(context);
				if(reqAgain!=0){
					new CalledDownloadActivityMark().execute();
				}else{
					new DownloadSubActMark(context).DownSubActMark();
				}	
			}	
		}
	}
	
	public void DownActivityMark(){
		sqlHandler.deleteTable("activitymark");
		new CalledDownloadActivityMark().execute();
	}
}
